  ...
  int open (ACE_Reactor *reactor)
  {
    ...
    if (reactor->register_handler (this,
      ...
      ACE_ERROR_RETURN ((LM_ERROR,
                        ...
                        -1);
    std::cout << "Client " << this->get_handle() << " connected" << std::endl;
    return 0;
  }
...
protected:
  ...
  virtual int handle_input (ACE_HANDLE)
  {
    ...
    ssize_t retval;
    switch (retval = this->cli_stream_.recv(buf, sizeof(Data)))
    {
    case -1:
      ACE_ERROR_RETURN ((LM_ERROR,
                         "%p bad read\n",
    ...
    case 696:
      ...
      fileHandle << data->str<<std::endl;
      
      this->cli_stream_.send_n("OK\0",3);
      break;

    default:
      ACE_ERROR_RETURN ((LM_ERROR,
                  "Bad message (fd = %d) : %s\n",
                  this->get_handle (),(char*)buf),
              -1);
    }
